...
#5,下载escheduler
RUN cd /opt && \
    wget https://github.com/apache/incubator-dolphinscheduler/archive/1.1.0.tar.gz && \
    tar -zxvf 1.1.0.tar.gz && \
    mv incubator-dolphinscheduler-1.1.0 easyscheduler_source && \
    rm -rf ./incubator-dolphinscheduler-1.1.0.tar.gz
...
RUN mkdir -p /opt/escheduler && \
    tar -zxvf /opt/easyscheduler_source/target/escheduler-1.1.0.tar.gz -C /opt/escheduler && \
